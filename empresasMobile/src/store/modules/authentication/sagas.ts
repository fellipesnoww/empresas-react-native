...
function* signInUser({ payload }: SignInUserRequest) {
  try {
    ...
    const loginResponse: AxiosResponse<ILoginResponse> = yield call(
      api.post,
    ...
    if (loginResponse?.data.success) {
      ...
      yield put(
        signInFailure({
          errors: loginResponse.data.errors,
          success: loginResponse.data.success,
        }),
      );
    }
    ...
    yield put(
      signInFailure({
        errors: ['Houveram erros ao realizar login'],
        success: false,
      }),
    );
  }
...
export default all([takeLatest(ActionTypes.signInRequest, signInUser)]);
